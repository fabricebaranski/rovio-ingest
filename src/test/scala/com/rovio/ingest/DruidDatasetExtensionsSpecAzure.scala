...
class DruidDatasetExtensionsSpecAzure extends AnyFlatSpec with Matchers with BeforeAndAfter with BeforeAndAfterEach {
  ...
  it should "save the dataset to druid on Azure" in {
    ...
    val options: Map[String, String] = DruidSourceBaseTest.getDataSourceOptions(DruidSourceBaseTest.PGSQL).asScala.toMap ++
